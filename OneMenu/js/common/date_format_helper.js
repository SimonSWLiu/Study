...
    MILLIS_PER_YEAR: 365 * 24 * 60 * 60 * 1000,
...
    }, 
	...
	/** 
     * 根据给定的两个Timestamp 
     * @params preTimestamp,rearTimestamp
     * @return 时间间隔 
     */   
	showInterval : function(preTimestamp, rearTimestamp){   

         try{   

			var str = ""

			var diff = rearTimestamp - preTimestamp;

			/*
			console.log(" showInterval --> diff : " + diff);
			console.log(" showInterval --> MILLIS_PER_YEAR : " + this.MILLIS_PER_YEAR);
			console.log(" showInterval --> MILLIS_PER_DAY : " + this.MILLIS_PER_DAY);
			console.log(" showInterval --> MILLIS_PER_HOUR : " + this.MILLIS_PER_HOUR);
			console.log(" showInterval --> MILLIS_PER_MIN : " + this.MILLIS_PER_MIN);
			console.log(" showInterval --> MILLIS_PER_SECOND : " + this.MILLIS_PER_SECOND);
			*/

			if(diff - this.MILLIS_PER_YEAR > 0) {
				var interval = Math.floor(diff / this.MILLIS_PER_YEAR);
				if (interval > 1) {
					str +=  interval + " years ago"
				}
				else{
					str +=  interval + " year ago"
				};
				
			}
			else if(diff - this.MILLIS_PER_DAY > 0) {
				var interval = Math.floor(diff / this.MILLIS_PER_DAY);
				if (interval > 1) {
					str +=  interval + " days ago"
				}
				else{
					str +=  interval + " day ago"
				};
			}
			else if(diff - this.MILLIS_PER_HOUR > 0) {
				var interval = Math.floor(diff / Math.floor(this.MILLIS_PER_HOUR));
				if (interval > 1) {
					str +=  interval + " hours ago"
				}
				else{
					str +=  interval + " hour ago"
				};
			}
			else if(diff - this.MILLIS_PER_MINUTE > 0) {
				var interval = Math.floor(diff / this.MILLIS_PER_MINUTE);
				if (interval > 1) {
					str +=  interval + " minutes ago"
				}
				else{
					str +=  interval + " minute ago"
				};
			}
