...
public class ConnPool {
	...
	public ConnPool(String jdbcDriver, String dbUrl, String dbUsername,
			String dbPassword) {
	...
	private boolean createConnections(int numConnections) throws SQLException {

		boolean result = false;
		List<PooledConnection> createConnections = new ArrayList<PooledConnection>();

		System.out.println("准备创建指定数目的数据库连接 --> " + numConnections);
		// 循环创建指定数目的数据库连接
		for (int x = 0; x < numConnections; x++) {
			...
			if (this.maxConnections > 0
				...
				System.out.println("无法创建,已达到数据库连接池的最大数据库连接数...");
				break;
			}
			...
			try {
				...
				createConnections.add(new PooledConnection(newConnection()));
		...
		if (createConnections.size() > 0) {
			connections.addAll(createConnections);
			result = true;
			System.out.println("己创建数据库连接数 --> " + createConnections.size());
		}
	...
	private Connection getFreeConnection() throws SQLException {
		...
		if (conn == null) {
			// 如果目前连接池中没有可用的连接
			// 创建一些连接
			if (createConnections(incrementalConnections)) {
	...
	public boolean testConnection(Connection conn) {
		...
		try {
			...
			System.out.println("此连接不可用,准备关闭 --> " + conn);
	...
	public void returnConnection(Connection conn) {
		...
		while (enumerate.hasMoreElements()) {
			...
			if (conn == pConn.getConnection()) {
				...
				System.out.println("返回到连接池中的连接对象 --> " + conn);
		...
		System.out.println("此时连接池中可用连接数 －－> " + getCurrentConnections());
